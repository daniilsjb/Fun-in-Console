...
class GreedyBestFirstSearch : public ConsoleGameEngine
{
	struct Node
	{
		bool obstacle = false;
	...
	{
		SetApplicationTitle(L"Greedy Best First Search");
	...
	bool OnUpdate(float elapsedTime) override
	{
		...
		{
			return abs(finish->i - node->i) + abs(finish->j - node->j);
		};
		...
		while (!frontier.empty())
		{
			...
			for (auto &neighbor : current->neighbors)
			{
				...
				{
					frontier.push_front(std::make_pair(neighbor, heuristic(neighbor)));
